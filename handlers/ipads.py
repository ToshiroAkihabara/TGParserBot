...
async def ipad(event: types.CallbackQuery):
    await bot.answer_callback_query(event.id)
    await bot.delete_message(event.from_user.id, event.message.message_id)
    await bot.send_message(event.from_user.id, f'Выберите модель📲:', reply_markup=user_markups.ipad())
    ...
    if value == 'ipad_':
        ...
        if len(data) > 0:
            try:
                ...
                await bot.send_message(event.from_user.id, 'Спасибо за Ваше терпение!\nМожет Вас заинтересует другая модель?☺️', reply_markup=user_markups.ipad())       
